...
layout(location=2) in uint inPaletteElementIndex;
...
layout(location=0) flat out uint vertexPaletteElementIndex;
layout(location=1) out float light;

void main()
{
	gl_Position = uProjection * uView * uModel * vec4(inPosition, 1.0);
	vertexPaletteElementIndex = inPaletteElementIndex;
