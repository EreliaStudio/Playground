...
- Story intent: Apply, refresh/remove and serialize statuses.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Status hooks run only on subscribed authoritative events.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Summons use normal CombatUnit readiness/stats/targeting pipeline.` is observed exactly; no private-state shortcut is used.
