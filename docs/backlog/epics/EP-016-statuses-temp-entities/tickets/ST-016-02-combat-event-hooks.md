...
- Story intent: Dispatch bounded status triggers deterministically.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `EverySecond uses Encounter Clock semantics.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Removing a status stops future hooks without leaving callbacks registered.` is observed exactly; no private-state shortcut is used.
