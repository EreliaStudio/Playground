...
- Story intent: Create encounter-owned walls/zones/traps/totems/portals/turrets.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Temporary walls/zones/traps do not mutate persistent terrain cells.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Hook ordering is deterministic/documented.` is observed exactly; no private-state shortcut is used.
