...
- The dependency contracts `EP-013, EP-015` used by this story are available; an epic dependency denotes a required contract, not global waterfall completion.
...
- Story intent: Create temporary CombatUnits through shared combat pipeline.
- Required dependency contracts: EP-013, EP-015.
- Epic behavioral catalogue: status stack, refresh, and replace rules; duration ownership; deterministic hook ordering and recursion protection; encounter ownership; temporary terrain-like effects without immutable-terrain mutation; summons using CombatUnit rules; teardown.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `All encounter-owned temporary entities are cleaned up at encounter end.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Presentation-only model/material effects can visualize statuses without becoming the status truth.` is observed exactly; no private-state shortcut is used.
