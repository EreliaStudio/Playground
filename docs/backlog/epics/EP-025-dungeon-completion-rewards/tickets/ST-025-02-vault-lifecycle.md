...
- Story intent: Unlock and claim generated rewards.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Vault becomes available only after completion.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Later completions still produce normal repeat rewards.` is observed exactly; no private-state shortcut is used.
