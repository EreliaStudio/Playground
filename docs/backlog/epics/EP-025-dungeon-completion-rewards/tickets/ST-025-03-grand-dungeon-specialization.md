...
- Story intent: Configure Grand Dungeon through normal dungeon framework.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Reward claim is idempotent per run/claim policy.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Leaving/resetting dungeon clears runtime state but not server progression.` is observed exactly; no private-state shortcut is used.
