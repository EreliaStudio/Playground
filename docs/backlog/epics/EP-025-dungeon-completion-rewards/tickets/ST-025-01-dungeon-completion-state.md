...
- The dependency contracts `EP-022, EP-023, EP-026` used by this story are available; an epic dependency denotes a required contract, not global waterfall completion.
...
- Story intent: Track objectives and completed run state.
- Required dependency contracts: EP-022, EP-023, EP-026.
- Epic behavioral catalogue: completion prerequisites; idempotent completion; vault availability and claims; runtime reset versus permanent progression; Grand Dungeon reuse; first unlock once; repeat rewards; reward independence from visual assets.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Completion only occurs after configured objective criteria.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `First successful Grand Dungeon completion emits exactly one progression unlock mutation.` is observed exactly; no private-state shortcut is used.
