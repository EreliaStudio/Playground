...
- Story intent: Atomically connect first success to portal unlock.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Grand Dungeon reuses ordinary dungeon instance/generation contracts.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Reward presentation asset cannot change reward contents.` is observed exactly; no private-state shortcut is used.
