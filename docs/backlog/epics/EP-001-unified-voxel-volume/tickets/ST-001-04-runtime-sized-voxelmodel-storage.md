...
- Story intent: Add a simple owning volume suitable for imported props/parts with arbitrary dimensions and smaller scale.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `An 8×8×16 volume at scale 0.1 reports 0.8×0.8×1.6 local extent within defined floating tolerance.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Volume access and scale/bounds calculations run headlessly and without Definition catalog/GPU ownership.` is observed exactly; no private-state shortcut is used.
