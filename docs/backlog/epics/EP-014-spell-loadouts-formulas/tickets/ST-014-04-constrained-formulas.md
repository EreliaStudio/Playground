...
- The dependency contracts `EP-013, EP-018` used by this story are available; an epic dependency denotes a required contract, not global waterfall completion.
...
- Story intent: Parse/validate/evaluate data-driven spell formulas.
- Required dependency contracts: EP-013, EP-018.
- Epic behavioral catalogue: persistent tradable spell items; eight loadout slots; duplicate and slot policy; ownership; Type and Tag requirements; atomic rejection; deterministic constrained formulas with no executable content code.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Tags themselves have no intrinsic effect.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Equipping/unequipping a spell does not alter voxel meshes unless the presentation asset itself explicitly changes cell geometry.` is observed exactly; no private-state shortcut is used.
