...
- Story intent: Evaluate Type/Tag requirements against equipped items.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Equipment Type/Tag requirements are validated from current loadout.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Formula result is deterministic for same authoritative inputs.` is observed exactly; no private-state shortcut is used.
