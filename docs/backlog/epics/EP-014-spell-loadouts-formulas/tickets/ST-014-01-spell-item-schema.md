...
- Story intent: Represent spells as physical item definitions/instances.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Spell item has persistent ownership and can be traded/stored.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Formula evaluator exposes only allowed variables/functions.` is observed exactly; no private-state shortcut is used.
