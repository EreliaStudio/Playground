...
- Story intent: Equip and validate exactly eight spell slots.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Hero exposes exactly eight equipped spell slots.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Malformed/cyclic/unsupported formula content fails validation.` is observed exactly; no private-state shortcut is used.
