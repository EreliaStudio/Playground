...
- Story intent: Emit combat/economy/dungeon/world/etc. telemetry.
- Required dependency contracts: EP-000.
- Epic behavioral catalogue: authoritative outcome events; versioned envelope; time and order metadata; sink failure isolation; offline mode; documented backpressure/drop behavior; profiler separation; no secrets or PII.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Telemetry failure never rolls back otherwise valid gameplay unless explicitly configured.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Dropped/backpressured analytics follows documented policy.` is observed exactly; no private-state shortcut is used.
