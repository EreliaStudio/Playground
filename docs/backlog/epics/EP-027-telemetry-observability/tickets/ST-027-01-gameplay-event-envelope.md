...
- Story intent: Define versioned authoritative event metadata.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Gameplay events originate from authoritative accepted outcomes.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Profiler can measure model compile/load/chunk bake/render preparation separately.` is observed exactly; no private-state shortcut is used.
