...
- Story intent: Instrument CPU/GPU/content pipeline/runtime hotspots.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Events use stable versioned schemas/IDs.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Offline/local runs can disable telemetry.` is observed exactly; no private-state shortcut is used.
