...
- Story intent: Buffer/export events without coupling to gameplay success.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `PII/secrets are not placed in generic event payloads.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Event ordering/time domain is explicit enough for later analysis.` is observed exactly; no private-state shortcut is used.
