...
- The dependency contracts `EP-004, EP-018, EP-024` used by this story are available; an epic dependency denotes a required contract, not global waterfall completion.
...
- Story intent: Represent server-wide depleted/available state.
- Required dependency contracts: EP-004, EP-018, EP-024.
- Epic behavioral catalogue: deterministic valid sites; dynamic availability separate from terrain; tool requirements; atomic yield/depletion; concurrent gathering; authoritative respawn time; persistence.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Successful gather transfers resources atomically then marks node depleted.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Node visual material/model may change with state without changing terrain.` is observed exactly; no private-state shortcut is used.
