...
- Story intent: Restore availability after configured world-clock duration.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Terrain VoxelDefinitions are unchanged by depletion.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Save/reload preserves depletion/respawn schedule.` is observed exactly; no private-state shortcut is used.
