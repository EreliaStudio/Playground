...
- Story intent: Validate tool/tier and grant yield atomically.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Gather validation checks required tool/tier.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Concurrent gather attempts cannot duplicate yield.` is observed exactly; no private-state shortcut is used.
