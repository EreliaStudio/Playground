...
- Story intent: Place deterministic valid spawn sites independent of availability state.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `One node depletion is visible to all players sharing server state.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Respawn timer uses authoritative world time.` is observed exactly; no private-state shortcut is used.
