...
- Story intent: Render imported barrel/weapon with stable local palette pattern/variation.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Palette Material terrain remains stable while camera moves and has no unintended Chunk-boundary seam.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Changing Hero/weapon transforms and Material palette leaves mesher invocation count unchanged.` is observed exactly; no private-state shortcut is used.
...
- [ ] One immutable model mesh is rendered with two compatible small Palettes; both images differ as expected while geometry, cache key, and mesher invocation count remain unchanged.
- [ ] Moving the renderable changes only its model transform; object-local sampling and Palette lookup remain attached to the object.
