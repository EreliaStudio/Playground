...
- Story intent: Build the required terrain + barrel + articulated Hero + weapon scene using the common volume/mesher path.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Imported small-scale barrel meshes through the same mesher and uses object-local Material sampling.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `A deliberate voxel edit to barrel or test Chunk invalidates/remeshes only required volume(s)/neighbors.` is observed exactly; no private-state shortcut is used.
...
- [ ] One scene draws Chunks with the shared WorldPalette and model/assembly parts with their selected smaller Palettes through the same VoxelMesh/shader contract.
- [ ] The command trace is program → mesh vertices/indices → required Palette SSBO → transform data → draw; no global GPU PaletteCollection or palette ID is present.
