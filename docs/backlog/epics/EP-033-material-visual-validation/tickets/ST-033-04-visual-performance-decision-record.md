...
- Story intent: Capture images/profiling and decide whether the multi-scale voxel/Material direction is acceptable before broad content production.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Articulated Hero renders from multiple small-scale VoxelModels and animates part transforms without remesh.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Profiling captures mesh counts, triangles, draw calls, build times and cache reuse for the complete scene.` is observed exactly; no private-state shortcut is used.
