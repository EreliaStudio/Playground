...
- Story intent: Initialize/spend/end AP and MP.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Only the deciding encounter pauses; Region Clock/other encounters continue.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Re-entry follows reinforcement rules.` is observed exactly; no private-state shortcut is used.
