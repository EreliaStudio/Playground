...
- Story intent: Resolve explicit flee attempts and outside-wait state.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `AP and MP are refreshed/consumed according to configured rules.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Defeated Heroes recover at 1 HP after battle per baseline.` is observed exactly; no private-state shortcut is used.
