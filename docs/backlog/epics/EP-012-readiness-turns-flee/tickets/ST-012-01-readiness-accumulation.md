...
- Story intent: Advance readiness from Encounter Clock and TurnInterval.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Lower TurnInterval reaches readiness sooner under same elapsed encounter time.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Owner fully detaches only when all owned participants have fled/been defeated.` is observed exactly; no private-state shortcut is used.
