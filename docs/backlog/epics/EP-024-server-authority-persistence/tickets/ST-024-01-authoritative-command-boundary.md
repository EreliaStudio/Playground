...
- The dependency contracts `EP-000, EP-004, EP-006` used by this story are available; an epic dependency denotes a required contract, not global waterfall completion.
...
- Story intent: Validate actor ownership/preconditions before domain mutation.
- Required dependency contracts: EP-000, EP-004, EP-006.
- Epic behavioral catalogue: authoritative mutation boundary; actor, ownership, and preconditions; no partial mutation; schema versions and stable IDs; multi-object transactions; crash recovery; idempotent retry; same domain API locally and on dedicated authority; no rendering dependency.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Accounts/inventory/Heroes/progression/infrastructure/resources/camps/market/dungeons/encounters/rewards mutate only through authority boundary.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Server can run without rendering or external asset-authoring/import tooling.` is observed exactly; no private-state shortcut is used.
