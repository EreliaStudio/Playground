...
- Story intent: Keep authority services runnable in-process first and separately later.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Snapshot/journal strategy can restore a consistent state after controlled failure fixture.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Authoritative state hash excludes presentation-only material animation.` is observed exactly; no private-state shortcut is used.
