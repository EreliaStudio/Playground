...
- Story intent: Group multi-object authoritative changes atomically.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Stable IDs and schema versions survive restart.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Idempotency/retry policy prevents duplicate transactional effects where applicable.` is observed exactly; no private-state shortcut is used.
