...
- Story intent: Version and persist shared domain state.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Rejected commands leave no partial persistent state.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Client-provided visual/material data cannot grant gameplay properties.` is observed exactly; no private-state shortcut is used.
