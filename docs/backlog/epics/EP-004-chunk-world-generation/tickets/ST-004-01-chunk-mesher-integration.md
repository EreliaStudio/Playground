# ST-004-01 — Chunk mesher integration

## Intent

Consume the EP-001 Chunk VoxelVolume adaptation and route it into the generic VoxelMesher without creating another adapter.
...
- Story intent: Consume the EP-001 Chunk VoxelVolume adaptation and route it into the generic VoxelMesher without creating another adapter.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Existing stable Chunk generation fixture is unchanged by adapting Chunk to the volume contract.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Replacing `Chunk::Baker` use with VoxelMesher + Chunk resolver preserves representative current mesh semantics.` is observed exactly; no private-state shortcut is used.
