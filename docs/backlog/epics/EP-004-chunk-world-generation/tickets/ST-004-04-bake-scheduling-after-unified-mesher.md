...
- Story intent: Route Chunk mesh requests through EP-032 while preserving dirty/version and neighbor invalidation semantics.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Missing/unavailable neighboring Chunk follows the documented current/fallback visibility policy deterministically.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Editing one Chunk increments/version-dirties it and required neighbor bake scheduling without mutating immutable generated source rules.` is observed exactly; no private-state shortcut is used.
