...
- Story intent: Keep seeded generation, request/publish/remove and regeneration behavior independent of rendering.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Chunk resolver maps an out-of-bounds local neighbor to the correct adjacent Chunk cell when available.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `World generation and Chunk collection run without Material/GPU/model-import dependencies.` is observed exactly; no private-state shortcut is used.
