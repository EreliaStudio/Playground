...
- The dependency contracts `EP-018, EP-024` used by this story are available; an epic dependency denotes a required contract, not global waterfall completion.
...
- Story intent: Select and buy eligible crafted listings for reward placement.
- Required dependency contracts: EP-018, EP-024.
- Epic behavioral catalogue: gold ledger; real-item reservation on listing; cancellation and expiry; atomic and concurrent purchase; real PvE reward budget; procurement of a real listing; seller payment; no double procurement.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Cancelled/expired listing returns ownership correctly.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Concurrent purchase/procurement of same listing resolves at most once.` is observed exactly; no private-state shortcut is used.
