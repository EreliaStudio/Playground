...
- Story intent: Represent funded PvE reward-source budget.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Purchase transfers item and gold atomically.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Pricing-abuse policy remains a decision gate but interface supports eligibility/fair-price filter.` is observed exactly; no private-state shortcut is used.
