...
- Story intent: Create/cancel/purchase player listings.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Listing transfers/reserves item without duplicating ownership.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Procured reward corresponds to a real eligible listing and seller receives gold.` is observed exactly; no private-state shortcut is used.
