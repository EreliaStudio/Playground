...
- Story intent: Track universal currency through auditable authoritative transactions.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Gold changes only through authoritative ledger operations.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `PvE reward source cannot exceed configured budget.` is observed exactly; no private-state shortcut is used.
