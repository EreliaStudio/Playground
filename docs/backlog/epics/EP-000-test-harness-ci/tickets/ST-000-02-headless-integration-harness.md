...
- Story intent: Provide a reusable application/runtime harness that instantiates domain services without presentation services.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Headless tests execute without creating a Sparkle Window, Surface, or OpenGL context.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Test data has stable seeds and versions so failures are reproducible locally and in CI.` is observed exactly; no private-state shortcut is used.
