...
- Story intent: Provide fixed workloads and reporting for regressions, leaks, lifecycle loops, and long-running simulations.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Voxel-volume/mesher/import fixtures can compare semantic outputs against stable expectations rather than raw byte layout where layout is allowed to evolve.` is observed exactly; no private-state shortcut is used.
