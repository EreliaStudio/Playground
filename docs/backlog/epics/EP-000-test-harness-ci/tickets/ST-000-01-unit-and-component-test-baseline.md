...
- Story intent: Standardize GoogleTest fixtures, naming, deterministic seeds, temporary directories, and failure diagnostics.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `A clean checkout can configure, build, and execute the complete automated test suite from one documented command path.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Performance fixtures report distributions for current Chunk bake, generalized VoxelMesher, imported-model load/mesh-cache, encounter step, and representative render preparation workloads.` is observed exactly; no private-state shortcut is used.
