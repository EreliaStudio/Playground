...
- Story intent: Create persistent blank Hero records with stable ownership.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `A player may own more than three Heroes while selecting exactly three active for an expedition when required.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Multiplayer party composition preserves each player’s own active Heroes.` is observed exactly; no private-state shortcut is used.
