...
- Story intent: Combine participating players’ active Heroes without transferring ownership.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Changing active squad does not duplicate/loss inventory ownership.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Headless server can create/manage Heroes without loading GPU meshes.` is observed exactly; no private-state shortcut is used.
