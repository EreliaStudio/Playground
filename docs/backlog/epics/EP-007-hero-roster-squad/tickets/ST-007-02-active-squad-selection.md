...
- Story intent: Select and validate expedition Heroes.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `No level/class/proficiency field is required to create a Hero.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Stable Hero identity survives save/reload.` is observed exactly; no private-state shortcut is used.
