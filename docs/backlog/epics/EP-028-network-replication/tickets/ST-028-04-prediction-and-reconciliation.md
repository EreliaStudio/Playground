...
- The dependency contracts `EP-024` used by this story are available; an epic dependency denotes a required contract, not global waterfall completion.
...
- Story intent: Reconcile continuous exploration movement under latency.
- Required dependency contracts: EP-024.
- Epic behavioral catalogue: session identity; no direct client mutation; relevant-state replication and interest management; deterministic-generation version matching and fallback; duplicate, reorder, and loss handling; reconnect; client-only prediction and reconciliation; no trust in visual IDs.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Mismatch falls back to authoritative compatible source policy.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `VoxelModel/assembly/material asset IDs replicate as presentation references, not trusted gameplay payloads.` is observed exactly; no private-state shortcut is used.
