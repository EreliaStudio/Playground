...
- Story intent: Track region/encounter relevance.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Immutable deterministic chunk data may be generated locally only when content/generation versions match.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Reconnect restores current state without duplicating entities/items.` is observed exactly; no private-state shortcut is used.
