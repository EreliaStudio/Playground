...
- Story intent: Authenticate/associate client session with player authority.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Client cannot authoritatively grant items/gold/progression/combat outcomes.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Movement prediction/reconciliation does not alter server truth.` is observed exactly; no private-state shortcut is used.
