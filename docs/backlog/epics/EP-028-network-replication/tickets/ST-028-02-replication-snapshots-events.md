...
- Story intent: Serialize relevant dynamic authoritative state.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Interest management limits replicated dynamic entities to relevant regions/encounters.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Duplicate/reordered/lost message fixtures preserve transactional invariants.` is observed exactly; no private-state shortcut is used.
