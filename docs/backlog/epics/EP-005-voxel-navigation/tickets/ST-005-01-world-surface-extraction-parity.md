...
- Story intent: Protect current/expected shape-based traversable-surface extraction through the voxel refactor.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Existing cube/slab/slope/stair world fixtures yield expected traversable surfaces after unified volume refactor.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Adding a decorative barrel VoxelModel does not create traversal cells unless an explicit world collision/obstacle system registers it.` is observed exactly; no private-state shortcut is used.
