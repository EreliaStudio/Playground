...
- The dependency contracts `EP-001, EP-004` used by this story are available; an epic dependency denotes a required contract, not global waterfall completion.
...
- Story intent: Expose valid world surfaces to encounter combat-cell construction without using render meshes as authority.
- Required dependency contracts: EP-001, EP-004.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `A visually identical Chunk at a different hypothetical voxel scale changes world dimensions consistently only when explicitly used as world geometry.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Chunk unload/reload regenerates equivalent traversal for identical authoritative cells.` is observed exactly; no private-state shortcut is used.
