...
- Story intent: Keep movable/model entities separate from static terrain graph while allowing explicit blockers later.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Two surfaces at different Y positions in one X/Z column remain distinct.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Material palette changes have zero traversal effect.` is observed exactly; no private-state shortcut is used.
