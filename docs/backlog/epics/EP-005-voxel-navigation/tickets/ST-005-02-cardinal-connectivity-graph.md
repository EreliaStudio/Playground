...
- Story intent: Connect compatible neighboring world surfaces with height/action metadata needed by exploration/combat.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Rotated/flipped world cells produce correspondingly transformed traversal geometry.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Hero limb/equipment VoxelModels never alter terrain navigation during animation.` is observed exactly; no private-state shortcut is used.
