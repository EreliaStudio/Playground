...
Define named assembly anchors, parent/child relationships, and local transforms independent of pure VoxelModel cell storage.
...
- Story intent: Define named assembly anchors, parent/child relationships, and local transforms independent of pure VoxelModel cell storage.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Torso/head/upper+lower limbs import and assemble into one Hero hierarchy.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Two Heroes can share part/model meshes while having independent poses/transforms.` is observed exactly; no private-state shortcut is used.
