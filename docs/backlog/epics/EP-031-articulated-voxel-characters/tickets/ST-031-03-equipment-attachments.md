...
- Story intent: Attach weapon/shield/helmet or other visible equipment to named hierarchy points.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Attack/spell pose can rotate hierarchy while weapon remains attached to expected point.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Visual pose change does not change authoritative collision unless explicit gameplay logic updates it.` is observed exactly; no private-state shortcut is used.
