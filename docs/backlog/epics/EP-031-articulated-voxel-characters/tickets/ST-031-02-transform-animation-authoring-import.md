...
- Story intent: Use mature tooling or declarative clips to author rigid-part transform keyframes.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Idle/walk clip transforms parts without any VoxelMesher call after meshes are ready.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Animation interruption/cross-clip switch never creates a cyclic/invalid hierarchy.` is observed exactly; no private-state shortcut is used.
