...
- Story intent: Create encounter-owned clocks with pause/resume.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Two encounters advance independently.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Clock serialization/reload preserves semantic scheduling.` is observed exactly; no private-state shortcut is used.
