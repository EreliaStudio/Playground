...
- Story intent: Implement explicit fixed-step Region Clock advancement.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Region Clock continues while one Encounter Clock is paused.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Transfer between time domains is atomic.` is observed exactly; no private-state shortcut is used.
