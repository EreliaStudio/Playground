...
- Story intent: Track exactly one simulation clock owner per entity.
- Required dependency contracts: EP-000.
- Epic behavioral catalogue: deterministic fixed-step Region Clock; invalid advancement; independent Encounter Clocks; isolated pause/resume; authoritative time-domain ownership; atomic world-to-encounter handoff; snapshot/reload/replay; the Encounter Clock remains internal simulation state.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Explicit fixed-step advancement is deterministic.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Long-step and equivalent repeated short-step fixtures produce documented equivalent behavior where required.` is observed exactly; no private-state shortcut is used.
