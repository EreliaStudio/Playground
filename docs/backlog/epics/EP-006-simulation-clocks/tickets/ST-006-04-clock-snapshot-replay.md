...
- Story intent: Serialize and replay clock state deterministically.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Entities belong to exactly one active time domain.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Presentation frame rate cannot change authoritative clock advancement.` is observed exactly; no private-state shortcut is used.
