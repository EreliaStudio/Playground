...
- Story intent: Validate selectable target cells/entities.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Min/max range boundaries are exact.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `AoE resolves deterministic target cell sets.` is observed exactly; no private-state shortcut is used.
