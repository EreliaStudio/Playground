...
- Story intent: Generate affected cells/targets deterministically.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `LoS uses authoritative combat/world geometry contract, not rendered pixels.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Invalid target command spends no AP and consumes no usage counter.` is observed exactly; no private-state shortcut is used.
