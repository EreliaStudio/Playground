...
- Story intent: Resolve obstruction in 3D cell space.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Fixed-range spells ignore only the modifiers they explicitly forbid.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `AP/cooldown/uses-per-turn/target/encounter are all enforced before mutation.` is observed exactly; no private-state shortcut is used.
