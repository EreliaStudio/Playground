...
- Story intent: Expose crafting/refining/enchantment/bank/etc. from configuration.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Service availability derives from building state.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Respawn Point selection is persistent and valid.` is observed exactly; no private-state shortcut is used.
