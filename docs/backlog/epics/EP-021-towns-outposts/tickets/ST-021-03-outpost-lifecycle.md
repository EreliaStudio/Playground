...
- The dependency contracts `EP-004, EP-019, EP-020, EP-024` used by this story are available; an epic dependency denotes a required contract, not global waterfall completion.
...
- Story intent: Activate, supply and deactivate temporary public outposts.
- Required dependency contracts: EP-004, EP-019, EP-020, EP-024.
- Epic behavioral catalogue: no aggregate Town Level; independent buildings; service gating; valid outpost sites; shared public state; upkeep and deactivation; respawn point; concurrent contributions and upgrades.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Outposts activate only at predefined valid crystal sites.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Infrastructure visual models can change by state/upgrade without modifying rules unexpectedly.` is observed exactly; no private-state shortcut is used.
