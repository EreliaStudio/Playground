...
- Story intent: Select/use valid persistent Respawn Points.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Outpost upkeep consumes configured resources and deactivates at zero.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Concurrent contributions/upgrades are transactionally safe.` is observed exactly; no private-state shortcut is used.
