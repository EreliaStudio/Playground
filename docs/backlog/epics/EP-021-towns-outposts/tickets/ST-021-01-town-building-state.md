...
- Story intent: Represent permanent towns and independent building upgrades.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Town buildings upgrade independently with no Town Level aggregate.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Public outpost service state is shared server-wide.` is observed exactly; no private-state shortcut is used.
