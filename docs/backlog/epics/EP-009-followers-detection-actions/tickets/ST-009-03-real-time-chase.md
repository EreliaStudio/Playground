...
- The dependency contracts `EP-005, EP-008` used by this story are available; an epic dependency denotes a required contract, not global waterfall completion.
...
- Story intent: Transition detected enemies into pursuit before engagement.
- Required dependency contracts: EP-005, EP-008.
- Epic behavioral catalogue: follow and hold; physical follower movement; unreachable traversal failure; jump, climb, and fall actions; deterministic awareness; unaware-to-aware-to-chase transition; world-clock chase before encounter; render detail is never gameplay authority.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Follower traversal attempts valid actions and holds if impossible.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Encounter start cancels uncommitted targeting according to handoff rules.` is observed exactly; no private-state shortcut is used.
