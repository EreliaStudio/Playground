...
- Story intent: Implement initial enemy awareness by vision.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Follow/hold changes behavior without teleporting followers.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Explicit jump/climb targeting does not pause Region Clock.` is observed exactly; no private-state shortcut is used.
