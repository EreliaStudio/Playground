...
- Story intent: Implement jump/climb target selection and follower replication rules.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Vision detection uses authoritative positions/occlusion inputs.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Follower visuals are independent of navigation state.` is observed exactly; no private-state shortcut is used.
