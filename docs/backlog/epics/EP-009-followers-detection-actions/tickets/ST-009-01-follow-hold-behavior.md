...
- Story intent: Implement physical follower locomotion modes.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Followers occupy real world positions and can trigger detection/traps.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Detection transitions to chase before encounter handoff.` is observed exactly; no private-state shortcut is used.
