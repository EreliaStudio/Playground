...
- Story intent: Inject deterministic RNG and critical modifiers.
- Required dependency contracts: EP-000.
- Epic behavioral catalogue: stat bounds and arithmetic overflow/underflow; ordered damage stages; deterministic critical RNG; reusable bounded effects; invalid-effect atomicity.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Defense uses selected diminishing-return formula once decision gate is resolved.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Boundary/overflow inputs fail or clamp only according to explicit contract.` is observed exactly; no private-state shortcut is used.
