...
- Story intent: Define bounded reusable gameplay effect operations.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Crit behavior is deterministic under injected RNG.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Summons/enemies can use same statistic primitives as Heroes.` is observed exactly; no private-state shortcut is used.
