...
- Story intent: Implement ordered damage computation.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Damage pipeline applies formula -> raw -> crit -> defense -> final in documented order.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Effect vocabulary rejects unknown operations in content.` is observed exactly; no private-state shortcut is used.
