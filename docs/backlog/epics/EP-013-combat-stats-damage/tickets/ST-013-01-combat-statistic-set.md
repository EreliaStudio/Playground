...
- Story intent: Define configurable authoritative statistic container.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Configured stat values serialize deterministically.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `No implicit Mana resource exists in baseline.` is observed exactly; no private-state shortcut is used.
