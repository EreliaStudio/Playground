...
- Story intent: Generate/store valid potential camp locations.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Later completions remain repeatable without duplicating progression mutation.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `World progression survives restart.` is observed exactly; no private-state shortcut is used.
