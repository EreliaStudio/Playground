...
- The dependency contracts `EP-004, EP-021, EP-024` used by this story are available; an epic dependency denotes a required contract, not global waterfall completion.
...
- Story intent: Persist server-wide unlocked transitions.
- Required dependency contracts: EP-004, EP-021, EP-024.
- Epic behavioral catalogue: deterministic WorldSeed derivation; server-wide portal unlock; first Grand Dungeon progression exactly once; repeat completion; camp sites separated from active camps; one-time clear reward; deterministic configurable replacement; restart persistence.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `First Grand Dungeon progression event permanently activates next-World portal exactly once.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Replacement camp activation follows configured deterministic/server policy.` is observed exactly; no private-state shortcut is used.
