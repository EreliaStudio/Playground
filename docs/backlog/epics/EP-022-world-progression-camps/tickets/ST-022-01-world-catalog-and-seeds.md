...
- Story intent: Represent multiple World identities and generation parameters.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `WorldSeed derives from ServerSeed + WorldID.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Clearing a camp rewards once then deactivates it.` is observed exactly; no private-state shortcut is used.
