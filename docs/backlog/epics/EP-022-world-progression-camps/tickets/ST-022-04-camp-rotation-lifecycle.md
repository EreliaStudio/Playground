...
- Story intent: Activate, clear, reward and replace overworld camps.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Camp sites are predefined/generated valid locations separate from active selection.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Portal/camp visual models do not define authoritative activation state.` is observed exactly; no private-state shortcut is used.
