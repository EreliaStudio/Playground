...
- Story intent: Derive stable layout key for current server rotation.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Same rotation re-entry produces same structural layout.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Generation rejects incompatible room connectors rather than overlap/corrupt space.` is observed exactly; no private-state shortcut is used.
