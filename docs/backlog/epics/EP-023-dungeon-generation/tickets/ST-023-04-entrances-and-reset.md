...
- Story intent: Instantiate per-group runtime from selected entrance and reset ephemeral state.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Runtime encounter/loot state resets on re-entry as specified.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Dungeon generation is headless and does not require GPU meshes/material resources.` is observed exactly; no private-state shortcut is used.
