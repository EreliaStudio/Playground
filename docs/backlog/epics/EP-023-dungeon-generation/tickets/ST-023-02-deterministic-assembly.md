...
- Story intent: Assemble valid room graph/layout using WFC/similar algorithm.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Generation uses ServerSeed, RotationEpoch, RotationDuration and DungeonID deterministically.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Multiple entrances map to distinct start positions in same generated instance.` is observed exactly; no private-state shortcut is used.
