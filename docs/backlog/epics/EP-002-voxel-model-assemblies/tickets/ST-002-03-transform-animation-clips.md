...
- Story intent: Define clips over articulated transforms with explicit duration/loop and optional presentation-only markers.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Changing entity/world transform never invokes the VoxelMesher.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Visual part hierarchy changes cannot modify authoritative HP/AP/collision/spell validity.` is observed exactly; no private-state shortcut is used.
...
- [ ] Advancing a clip changes only named anchor local transforms and derived part matrices; voxel cells, mesh bytes, cache identities, and mesher invocation count remain unchanged.
- [ ] A clip targeting an unknown anchor, duplicate key time, invalid time, or unsupported interpolation is rejected before playback state changes.
- [ ] Animation data syntax and interpolation rules are user-approved before serialization is frozen.
