...
- Story intent: Represent parent/child part hierarchy for torso/head/limbs using transforms over independent VoxelModels.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Reusing one VoxelModel asset across many entities reuses cached mesh data rather than duplicating geometry per transform.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Sword/shield/helmet model attachments follow named parent transforms and can be swapped independently.` is observed exactly; no private-state shortcut is used.
...
- [ ] A fixture with anchors `Root` and child `LeftShoulder`, plus torso and upper-arm model references, evaluates exact local-to-world transforms in parent-before-child order.
- [ ] Missing parent, duplicate anchor name, cycle, missing model, or part referencing an unknown anchor rejects the entire assembly and preserves the last valid instance.
- [ ] The assembly schema/sample is user-approved before its syntax is frozen; anchors never migrate into the referenced model files.
