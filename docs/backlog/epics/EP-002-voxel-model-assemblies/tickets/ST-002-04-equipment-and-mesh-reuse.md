...
- Story intent: Attach independent equipment VoxelModels and cache meshes across entities/poses.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Playing a walk/attack clip over rigid-part transforms never invokes the VoxelMesher.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Missing part, cyclic hierarchy or missing attachment point fails predictably before publishing the assembly.` is observed exactly; no private-state shortcut is used.
...
- [ ] One sword VoxelModel attaches to a user-approved hand-equipment anchor with a local transform; detach restores the pre-attachment part set without remeshing character or sword.
- [ ] Helmet/chest/multi-piece equipment reuses model parts and named anchors; no complete duplicate character model is required.
- [ ] Swapping an instance Palette changes visible material lookup but preserves shared mesh cache identity and does not invoke VoxelMesher.
- [ ] The anchor vocabulary is presented to the user and approved before being standardized.
