...
- The dependency contracts `EP-018, EP-019` used by this story are available; an epic dependency denotes a required contract, not global waterfall completion.
...
- Story intent: Consume costs and resolve success/failure atomically.
- Required dependency contracts: EP-018, EP-019.
- Epic behavioral catalogue: recipe validation; atomic consumption/output; higher tiers without profession levels; deterministic authoritative RNG; known enchantment transformation; configured success/failure where failure consumes cost and leaves target unchanged; Tag add/remove/replace; Type/stat tradeoffs; no town dependency.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Finished persistent equipment/spells originate from crafting/trading path, not ordinary monster generation.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Crafted item visual/material variant can be selected without remeshing geometry when its voxel cells are unchanged.` is observed exactly; no private-state shortcut is used.
