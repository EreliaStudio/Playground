...
- Story intent: Create finished item instances from recipes and configured ranges.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Higher tiers can require increasing raw material quantities without profession levels.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Failure consumes configured cost and leaves target item unchanged in base rule.` is observed exactly; no private-state shortcut is used.
