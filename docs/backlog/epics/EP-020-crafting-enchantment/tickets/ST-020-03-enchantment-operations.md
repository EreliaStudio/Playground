...
- Story intent: Define deterministic tag/stat/type transformations.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Crafting rolls configured stat ranges through authoritative deterministic RNG source.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Tag add/remove/replace enforces tradeoffs/type changes explicitly.` is observed exactly; no private-state shortcut is used.
