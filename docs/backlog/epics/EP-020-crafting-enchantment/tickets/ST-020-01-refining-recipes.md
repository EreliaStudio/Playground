...
- Story intent: Convert raw to refined resources.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Refining consumes recipe inputs atomically and creates configured outputs.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Enchantment intended result is deterministic for selected operation.` is observed exactly; no private-state shortcut is used.
