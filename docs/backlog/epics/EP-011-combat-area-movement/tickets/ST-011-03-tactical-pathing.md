...
- Story intent: Resolve MP-cost movement through valid combat cells.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Elastic area equals union of configured radius around combatants.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `World base terrain remains immutable during combat.` is observed exactly; no private-state shortcut is used.
