...
- The dependency contracts `EP-005, EP-010` used by this story are available; an epic dependency denotes a required contract, not global waterfall completion.
...
- Story intent: Maintain combat area around participants.
- Required dependency contracts: EP-005, EP-010.
- Epic behavioral catalogue: world surfaces projected to 1x1 horizontal combat cells; multiple heights at one X/Z; dynamic occupancy; elastic area; deterministic path and MP cost; invalid movement rejection without MP consumption.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Cells retain 1x1 horizontal gameplay footprint regardless of visual VoxelModel resolution/voxel scale.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Cell graph respects slope/slab traversal metadata.` is observed exactly; no private-state shortcut is used.
