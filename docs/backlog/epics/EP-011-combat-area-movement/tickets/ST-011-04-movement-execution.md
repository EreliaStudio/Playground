...
- Story intent: Commit movement and events through Encounter Clock rules.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Adding/removing/moving combatants updates area deterministically.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Rendering mesh detail does not create extra tactical cells.` is observed exactly; no private-state shortcut is used.
