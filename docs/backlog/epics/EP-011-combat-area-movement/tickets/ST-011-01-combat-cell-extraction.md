...
- Story intent: Convert traversal surfaces into encounter cells.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Combat cell positions correspond to valid traversable voxel surfaces.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Movement cannot target invalid/unreachable cells.` is observed exactly; no private-state shortcut is used.
