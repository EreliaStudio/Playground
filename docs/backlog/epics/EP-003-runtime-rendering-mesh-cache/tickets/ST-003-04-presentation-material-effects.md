...
- Story intent: Support per-instance tint/effect parameters and leave room for emissive/metallic/transparency later.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Palette/material parameter changes do not call the VoxelMesher.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Cache unload/reload releases GPU resources without invalidating headless volume/model data.` is observed exactly; no private-state shortcut is used.
