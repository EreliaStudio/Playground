...
- Story intent: Upload/index/cache VoxelMesher output independent of whether the source was Chunk or VoxelModel.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Current textured chunk fixture renders through generalized mesher with equivalent Shape transforms/material-slot/UV semantics.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `World-space terrain variation remains continuous across Chunk boundaries where configured.` is observed exactly; no private-state shortcut is used.
