...
- Story intent: Prototype deterministic palette variation with explicit world/object/local sampling intent.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Same VoxelModel mesh is reused by multiple transforms/entities.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Moving the camera alone cannot change deterministic palette-cell assignment/pattern phase.` is observed exactly; no private-state shortcut is used.
