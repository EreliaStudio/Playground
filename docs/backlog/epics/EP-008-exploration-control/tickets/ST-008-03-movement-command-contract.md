...
- Story intent: Separate sampled client intent from authoritative movement result.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Camera state does not affect authoritative movement validity.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Top-down targeting view can be entered/exited without pausing Region Clock.` is observed exactly; no private-state shortcut is used.
