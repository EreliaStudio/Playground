...
- The dependency contracts `EP-004, EP-005, EP-007` used by this story are available; an epic dependency denotes a required contract, not global waterfall completion.
...
- Story intent: Switch presentation mode for explicit exploration actions.
- Required dependency contracts: EP-004, EP-005, EP-007.
- Epic behavioral catalogue: movement intent separated from authority; collision independent of render mesh; tunneling prevention; deterministic headless movement; camera independence from authoritative position; top-down targeting as presentation/input state.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Changing visual animation pose cannot move the authoritative collision shape unless explicit gameplay logic updates it.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Headless movement validation works without camera.` is observed exactly; no private-state shortcut is used.
