...
- Story intent: Implement authoritative-friendly continuous movement/collision.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Movement consumes continuous input rather than node-to-node commands.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Movement is deterministic enough under the chosen authoritative/prediction contract.` is observed exactly; no private-state shortcut is used.
