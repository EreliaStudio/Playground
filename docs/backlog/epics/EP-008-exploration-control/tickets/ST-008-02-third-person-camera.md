...
- Story intent: Implement exploration follow/orbit camera.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Character controller collides with authoritative world collision, not articulated visual VoxelModel meshes by default.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Blocked movement cannot tunnel through fixture geometry.` is observed exactly; no private-state shortcut is used.
