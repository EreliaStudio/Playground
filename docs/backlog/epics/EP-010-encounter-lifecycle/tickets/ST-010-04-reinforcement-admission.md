...
- The dependency contracts `EP-006, EP-009` used by this story are available; an epic dependency denotes a required contract, not global waterfall completion.
...
- Story intent: Admit later valid participants without restarting the encounter.
- Required dependency contracts: EP-006, EP-009.
- Epic behavioral catalogue: one encounter identity; initial participants and world positions; no double membership; outstanding-action policy; atomic time-domain transfer; Surprise from awareness; readiness initialization; reinforcement join with zero readiness; failed handoff rollback.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Each participant leaves Region Clock ownership and enters exactly one Encounter Clock atomically.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Failed handoff cannot strand an entity between world and encounter ownership.` is observed exactly; no private-state shortcut is used.
