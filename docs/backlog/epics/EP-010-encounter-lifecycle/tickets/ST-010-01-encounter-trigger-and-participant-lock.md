...
- Story intent: Determine initial participants and create encounter identity.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Initial participants are locked exactly once during handoff.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Unaware defender state grants the documented surprise readiness to the attacking side.` is observed exactly; no private-state shortcut is used.
