...
- Story intent: Initialize readiness based on awareness relationship.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Final world positions are snapshotted before combat cells are derived.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Reinforcement readiness begins at zero/fresh TurnInterval.` is observed exactly; no private-state shortcut is used.
