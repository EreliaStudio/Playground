...
- Story intent: Complete/cancel world actions and transfer time-domain ownership.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Uncommitted targeting is cancelled while committed actions finish.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Later arrivals do not restart initial synchronization.` is observed exactly; no private-state shortcut is used.
