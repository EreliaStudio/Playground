...
- Story intent: Allow optional serialized/cacheable mesher output without making it the authoritative model representation.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Unknown schema version or missing Definition fails before partial asset publication.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Headless server can load authoritative gameplay content without loading presentation-only model/material GPU data.` is observed exactly; no private-state shortcut is used.
