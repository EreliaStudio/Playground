...
Version imported asset mapping, assembly anchor hierarchy, attachments, model references, and source provenance.
...
- Story intent: Version imported asset mapping, assembly anchor hierarchy, attachments, model references, and source provenance.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Stable Definition IDs referenced by imported model cells resolve identically after content reload.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Material sampling-space/configuration fields round-trip deterministically.` is observed exactly; no private-state shortcut is used.
