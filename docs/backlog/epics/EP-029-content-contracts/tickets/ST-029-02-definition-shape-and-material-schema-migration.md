...
- Story intent: Evolve current atlas-bound definitions toward Material bindings without breaking current content fixtures.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `A runtime-sized model volume serializes/reloads dimensions, voxelSize and packed cells without semantic change.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Articulation/attachment metadata detects cycles/missing part IDs before runtime use.` is observed exactly; no private-state shortcut is used.
