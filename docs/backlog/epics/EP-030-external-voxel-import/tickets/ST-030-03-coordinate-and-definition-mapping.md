...
- Story intent: Make axis/origin/palette/Definition conversion explicit and testable.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Imported palette/color entries map deterministically to configured Voxel::Definition IDs.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Imported VoxelModel meshes through EP-032 with no model-specific mesher.` is observed exactly; no private-state shortcut is used.
