...
- Story intent: Evaluate mature existing voxel/3D tools and common formats for Erelia model production.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Tool/format spike compares at least viable candidates against documented requirements and records the selected first path.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Invalid/oversized source or unknown mapping fails without replacing last valid asset.` is observed exactly; no private-state shortcut is used.
