...
- The dependency contracts `EP-001, EP-029` used by this story are available; an epic dependency denotes a required contract, not global waterfall completion.
...
- Story intent: Integrate import/validation into build/content workflow with stable diagnostics and atomic output.
- Required dependency contracts: EP-001, EP-029.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Axis/origin conversion fixture produces expected orientation with no mirror/rotation surprise.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Importer executes without Sparkle Window/Surface/OpenGL and does not require a custom editor executable.` is observed exactly; no private-state shortcut is used.
