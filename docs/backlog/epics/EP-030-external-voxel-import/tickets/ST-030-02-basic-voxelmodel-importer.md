...
- Story intent: Import dimensions/cells/palette mapping into EP-001 runtime-sized VoxelModel.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `A small barrel authored externally imports to expected dimensions, voxelSize and Cell coordinates.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Re-import of identical source/mapping/converter version produces semantically identical VoxelModel.` is observed exactly; no private-state shortcut is used.
