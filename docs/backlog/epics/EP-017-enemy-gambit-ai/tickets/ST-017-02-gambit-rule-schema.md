...
- Story intent: Serialize and validate Condition → Target → Reaction entries.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Gambit rules are evaluated in deterministic priority order.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `AI cannot bypass AP/targeting/cooldown rules.` is observed exactly; no private-state shortcut is used.
