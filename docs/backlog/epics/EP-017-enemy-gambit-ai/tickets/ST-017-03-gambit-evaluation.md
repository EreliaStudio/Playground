...
- Story intent: Choose an action from ordered rules.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Condition/target/reaction content is validated before runtime.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Same state and deterministic RNG produce same chosen command.` is observed exactly; no private-state shortcut is used.
