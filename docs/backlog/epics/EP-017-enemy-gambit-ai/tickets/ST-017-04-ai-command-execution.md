...
- The dependency contracts `EP-012, EP-015, EP-016` used by this story are available; an epic dependency denotes a required contract, not global waterfall completion.
...
- Story intent: Submit selected actions through normal combat command validation.
- Required dependency contracts: EP-012, EP-015, EP-016.
- Epic behavioral catalogue: enemy combat definitions; ordered rules; invalid schema rejection; first-valid semantics; safe fallback; ordinary command validation; deterministic results; no AI effect from presentation assets.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `First valid rule chooses action according to documented semantics.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Enemy visual model/palette has no influence on decision logic.` is observed exactly; no private-state shortcut is used.
