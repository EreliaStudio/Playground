...
- Story intent: Define authored enemy stats/spells/visual reference.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Enemy stats/spells do not require equipment ownership.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `No valid rule resolves safe fallback/end-turn policy.` is observed exactly; no private-state shortcut is used.
