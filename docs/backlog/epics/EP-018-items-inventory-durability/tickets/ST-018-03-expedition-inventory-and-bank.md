...
- Story intent: Move items atomically between player storage locations.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Item has exactly one Type and zero or more Tags.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Repair changes durability according to decision-gated rules without duplicating item.` is observed exactly; no private-state shortcut is used.
