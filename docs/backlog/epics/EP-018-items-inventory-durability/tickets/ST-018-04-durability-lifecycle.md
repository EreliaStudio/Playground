...
- The dependency contracts `EP-007, EP-024` used by this story are available; an epic dependency denotes a required contract, not global waterfall completion.
...
- Story intent: Damage, repair and destroy equipment using configurable rules.
- Required dependency contracts: EP-007, EP-024.
- Epic behavioral catalogue: stable item identity; exactly one authoritative location; atomic moves and capacity rejection; hand and two-hand occupancy; Type and Tags; bank identity; zero-durability disappearance baseline; visual/stat independence.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Fixed-size expedition inventory rejects overflow atomically.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Visual item VoxelModel/material presentation can vary independently of the authoritative item stat instance.` is observed exactly; no private-state shortcut is used.
