...
- Story intent: Equip/unequip with hand occupancy validation.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Equipment validates Main/Off Hand, armor/accessory slots and two-handed occupancy.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Durability reaching zero permanently destroys equipment per configured rule.` is observed exactly; no private-state shortcut is used.
