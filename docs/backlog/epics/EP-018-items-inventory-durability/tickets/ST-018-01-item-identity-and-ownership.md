...
- Story intent: Create persistent item instances and one-location invariant.
...
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `One item has exactly one authoritative owner/location.` is observed exactly; no private-state shortcut is used.
- [ ] Given the declared fixture and valid dependency state, when the public operation is performed, `Bank moves preserve item identity and ownership.` is observed exactly; no private-state shortcut is used.
