# Illustration 14 — Encounter Handoff Synchronization

**Target output file:** `images/14_encounter_handoff.png`  
**GDD section:** 14.1 Initial Participant Synchronization  
**Suggested placement:** After the initial synchronization explanation.  
**Intended GDD size:** Half-width  
**Suggested canvas:** 2.3:1 landscape

## Purpose

Visualize how initial participants finish committed actions, freeze at their real final positions, and are then handed to tactical simulation.
...
Create a **four-stage sequence** using the same small group of Heroes and enemies.

Stage 1:
- the encounter has just been triggered;
- one participant is already in the middle of a committed movement or attack action;
- other initial participants are nearby.

Stage 2:
- idle participants have stopped;
- the committed participant visibly finishes its action.

Stage 3:
- every initial participant is frozen at its actual final world position.

Stage 4:
- tactical combat cells resolve beneath/around those final positions;
- participants are now clearly inside tactical combat.

Make the positions consistent from frame to frame so the handoff feels spatially continuous rather than like units were teleported.
...
No loading-screen transition, no arbitrary repositioning, no early tactical teleport before the committed action finishes.
...
Create **Illustration 14 — Encounter Handoff Synchronization** for the Erelia Game Design Document.
