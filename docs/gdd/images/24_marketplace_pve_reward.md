# Illustration 24 — Marketplace-Funded PvE Reward

**Target output file:** `images/24_marketplace_pve_reward.png`  
**GDD section:** 34. Marketplace-Funded PvE Rewards  
**Suggested placement:** After Section 34.2 Procurement Loop.  
**Intended GDD size:** Full-width  
**Suggested canvas:** 2:1 landscape

## Purpose

Show item provenance: finished PvE rewards can still originate from player crafting because the game buys player-listed objects from the marketplace.
...
Create a restrained **object-flow relationship illustration**.

Left:
- a player-crafted finished item, such as a sword or spell object, is placed into a small marketplace listing/stall representation by a player seller.

Center:
- a dungeon reward source is represented by a simple vault/chest with a budget marker or coin pile;
- a thin arrow connects the reward procurement process toward an eligible marketplace listing.

Right / return paths:
- gold flows to the seller;
- the **same identifiable player-crafted item** moves from the marketplace into the dungeon chest/vault as the eventual PvE reward.

The emphasis must be that the finished item was not spawned from nothing.

## Text inside the image

Only `Market`, `Gold`, `Vault` if needed.

## Important exclusions

No dense marketplace UI, no price charts, no loot-table spreadsheet, no monster directly generating the finished sword.
...
Create **Illustration 24 — Marketplace-Funded PvE Reward** for the Erelia Game Design Document.
