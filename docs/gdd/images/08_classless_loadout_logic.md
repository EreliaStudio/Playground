# Illustration 08 — Classless Loadout Logic

**Target output file:** `images/08_classless_loadout_logic.png`  
**GDD section:** 8. Item Type / Item Tags and 9. Eight Spell Slots / Equipment Requirements  
**Suggested placement:** At the transition between Sections 8 and 9, or after 9.3.  
**Intended GDD size:** Full-width  
**Suggested canvas:** 16:9 landscape

## Purpose

Show how equipment semantics enable spells and create a Hero's role without predefined classes.
...
Build a restrained **relationship illustration** around one central Hero silhouette.

Around the Hero, show equipped items in their natural slots:
- a shield clearly associated with item Type `Shield`;
- another piece of equipment carrying a small `Flame` Tag marker;
- optionally a different item carrying `Cursed`.

To one side, draw exactly **eight spell slots** as small cards/icons.

Use thin hand-drawn connector lines from the relevant equipment properties toward specific spell cards:
- one spell can require the shield;
- another can require a Flame-tagged equipped item;
- another can require Cursed.

One spell may appear unavailable/ghosted because its requirement is not currently satisfied.

The Hero must not be labelled with any class or role. The composition should communicate that the loadout creates the build.

## Text inside the image

Only short tokens such as `Shield`, `Flame`, `Cursed`, and very short spell names if necessary.

## Important exclusions

No class wheel, skill tree, Feat tree, level requirement, RPG archetype labels, or implication that Tags produce intrinsic effects on their own.
...
Create **Illustration 08 — Classless Loadout Logic** for the Erelia Game Design Document.
