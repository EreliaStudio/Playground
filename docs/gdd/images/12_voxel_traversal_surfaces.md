# Illustration 12 — Voxel Traversal Surfaces

**Target output file:** `images/12_voxel_traversal_surfaces.png`  
**GDD section:** 12.1 Walking Traversal Graph / 12.2 Multiple Traversable Surfaces per Column  
**Suggested placement:** After Section 12.2.  
**Intended GDD size:** Full-width  
**Suggested canvas:** 2:1 landscape

## Purpose

Explain the actual geometry behind traversal and why navigation/combat nodes retain a full 3D voxel position.
...
Compose two related **technical pencil sketches**.

Left half:
- show several representative voxel shapes: full cube, slab, stair, slope;
- indicate the traversable standing surface and small edge-height markers;
- use tiny connection lines between compatible neighboring surfaces to show how walking links are formed;
- if directional labels are useful, keep them very small.

Right half:
- draw a cutaway through a bridge, cave, balcony or stacked-floor arrangement;
- one traversable position exists on the ground beneath;
- another exists directly above on the bridge deck, sharing the same horizontal x/z column but different vertical position;
- clearly show that these are distinct valid navigation/combat positions.

This should feel like a design-engineering sketch, not a player HUD.

## Text inside the image

Only tiny `+X`, `-X`, `+Z`, `-Z` labels if necessary.

## Important exclusions

Do not collapse the navigation to a flat 2D grid. Do not imply that only full cubes are traversable.
...
Create **Illustration 12 — Voxel Traversal Surfaces** for the Erelia Game Design Document.
